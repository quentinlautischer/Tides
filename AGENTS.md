...
- `Controllers/ObservationsController.cs` - resolves an iNaturalist observation (`GET /api/observations?reference=`), taking either the observation URL or a bare id.
- `Services/INaturalistService.cs` - the iNaturalist lookup behind it (the leading I is their product name, not the interface prefix; the interface is `IObservationLookup`). Caches for 24h and self-throttles to one request a second, since iNaturalist asks for 60/minute or fewer. It exists server-side rather than in the SPA because iNaturalist asks callers to identify themselves with a User-Agent, which a browser will not let a page set.
  It returns the sighting's time as a **naive local wall clock with no offset**, matching what the tide endpoints emit - the client builds its chart target from bare date and time components, so an offset-bearing instant would land the marker off by the reader's UTC offset.
